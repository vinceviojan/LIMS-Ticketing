        ...
        <div v-if="displayMode === 'card'" class="row q-col-gutter-lg">
          ...
          >
            <q-card
              ...
              :class="{ 'ticket-card--selected': selectable && isSelected(ticket) }"
              flat
              ...
              <div class="q-pa-md col flex column justify-between" style="overflow: hidden; height: 100%;">
                ...
                <div class="row items-center justify-between dashed-top q-pt-sm no-wrap" style="gap: 8px; margin-top: auto;">
                  ...
                  <div class="row items-center gap-xs flex-shrink-0">
                    ...
                    <!-- Rated Star Pill -->
                    <q-chip v-if="ticket.rating" dense color="amber-1" text-color="amber-9" icon="star" class="text-weight-bold" style="font-size: 0.72rem; margin: 0;">
                      {{ ticket.rating }}/5
                    </q-chip>
                    <!-- Unrated Action Pill -->
                    <q-chip v-else-if="['RESOLVED', 'CLOSE'].includes(ticket.status)" dense color="amber-9" text-color="white" icon="rate_review" class="text-weight-bold" style="font-size: 0.72rem; margin: 0;">
                      Rate
                    </q-chip>
                  </div>
                </div>
            ...
            <template v-slot:body-cell-status="props">
              <q-td :props="props" class="text-center">
                <div class="row items-center justify-center gap-xs">
                  ...
                  <!-- Rated Star Pill -->
                  <q-chip v-if="props.row.rating" dense color="amber-1" text-color="amber-9" icon="star" class="text-weight-bold" style="font-size: 0.72rem; margin: 0;">
                    {{ props.row.rating }}/5
                  </q-chip>
                  <!-- Unrated Action Pill -->
                  <q-chip v-else-if="['RESOLVED', 'CLOSE'].includes(props.row.status)" dense color="amber-9" text-color="white" icon="rate_review" class="text-weight-bold" style="font-size: 0.72rem; margin: 0;">
                    Rate
                  </q-chip>
                </div>
              </q-td>
  ...
  // q-table's v-model:selected always wants an array, even in 'single' mode
  // (Quasar just enforces length <= 1 for us). We keep that array internally
  // but only ever expose a single ticket (or null) to the outside world, so
  // consumers of @update:selected don't have to unwrap a 1-item array.
  const selectedTickets = ref([])

  const selectedTicket = computed(() => selectedTickets.value[0] ?? null)

  watch(selectedTickets, () => {
    emit('update:selected', selectedTicket.value)
  })

  function isSelected(ticket) {
    return selectedTicket.value?.id === ticket.id
  }

  // Card view has no built-in q-table selection model, so clicking a card
  // both selects it (single-select, same as table view) and opens it.
  function handleCardClick(ticket) {
    if (props.selectable) {
      selectedTickets.value = isSelected(ticket) ? [] : [ticket]
    }
  ...
  const getStatusStyle = (status) => {
    ...
    if (s === 'ON-GOING') return { bg: '#eff6ff', color: '#1d4ed8', border: '#bfdbfe' }
