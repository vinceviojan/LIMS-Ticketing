      ...
      <div class="panel">
        ...
        <div class="donut-wrap" v-else>
          <svg class="donut" viewBox="0 0 120 120">
            ...
            />
            <text x="60" y="56" text-anchor="middle" dy=".35em" class="donut__num">
              {{ totalUsers }}
            </text>
      ...
      <div class="panel">
        ...
        <div class="donut-wrap">
          <svg class="donut" viewBox="0 0 120 120">
            <circle cx="60" cy="60" r="48" fill="none" stroke="#f3f4f6" stroke-width="14" />
            <circle
              v-for="(seg, i) in ticketStatusDonut"
              :key="i"
              cx="60"
              cy="60"
              r="48"
      ...
      <div class="panel">
        ...
        <div class="donut-wrap">
          <svg class="donut" viewBox="0 0 120 120">
            <circle cx="60" cy="60" r="48" fill="none" stroke="#f3f4f6" stroke-width="14" />
            <circle
              v-for="(seg, i) in ticketPriorityDonut"
              :key="i"
              cx="60"
              cy="60"
              r="48"
      ...
      <div class="panel">
        <div class="panel__head">
          <div class="panel__head-left">
            ...
            <span class="panel__sub">Top 5 registered categories</span>
          </div>
        ...
        <table class="clean-table" v-else-if="categories.length">
          <thead>
            <tr>
              <th class="clean-table__th clean-table__th--num">#</th>
              <th class="clean-table__th">Category</th>
              <th class="clean-table__th clean-table__th--right">Type</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(cat, i) in categories.slice(0, 5)" :key="cat.id" class="clean-table__row">
              <td class="clean-table__td clean-table__td--rank">{{ i + 1 }}</td>
              <td class="clean-table__td">
                <div class="clean-table__cell-main">{{ cat.categories }}</div>
              </td>
              <td class="clean-table__td clean-table__td--right">
                <span class="type-chip">{{ cat.type }}</span>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div class="panel">
        <div class="panel__head">
          <div class="panel__head-left">
            ...
            <span class="panel__sub">Last 5 registered accounts</span>
          </div>
        ...
        <table class="clean-table" v-else-if="recentUsers.length">
          <thead>
            <tr>
              <th class="clean-table__th">User</th>
              <th class="clean-table__th clean-table__th--right">Role</th>
              <th class="clean-table__th clean-table__th--right">Status</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="u in recentUsers" :key="u.id" class="clean-table__row">
              <td class="clean-table__td">
                <div class="clean-table__user">
                  <div
                    class="clean-table__avatar"
                    :class="`clean-table__avatar--${(u.role || 'user').toLowerCase()}`"
                  >
                    {{ ((u.first_name || '')[0] || '') + ((u.last_name || '')[0] || '') }}
                  </div>
                  <div class="clean-table__user-info">
                    <span class="clean-table__cell-main">{{ u.first_name }} {{ u.last_name }}</span>
                    <span class="clean-table__cell-sub">{{ u.email }}</span>
                  </div>
                </div>
              </td>
              <td class="clean-table__td clean-table__td--right">
                <span class="role-badge" :class="`role-badge--${(u.role || '').toLowerCase()}`">{{
                  u.role
                }}</span>
              </td>
              <td class="clean-table__td clean-table__td--right">
                <span class="status-pill" :class="`status-pill--${(u.status || '').toLowerCase()}`">
...
const loading = computed(
  () => usersLoading.value || categoriesLoading.value || analyticsLoading.value,
)

// ── KPIs (computed from live data) ────────────────────────────
const totalUsers = computed(() => allUsers.value.length)
const activeUsers = computed(
  () => allUsers.value.filter((u) => (u.status || '').toUpperCase() === 'ACTIVE').length,
)
...
const roles = computed(() => {
  const map = {
    ADMIN: { label: 'Admin', color: '#3b82f6', count: 0 },
    STAFF: { label: 'Staff', color: '#a855f7', count: 0 },
    USER: { label: 'User', color: '#009747', count: 0 },
  }
...
const ticketStatusData = computed(() =>
  Object.entries(analytics.value.tickets_by_status || {}).map(([status, count]) => ({
...
const totalTicketsStatus = computed(() =>
  ticketStatusData.value.reduce((acc, curr) => acc + curr.count, 0),
)
...
const ticketPriorityData = computed(() =>
  Object.entries(analytics.value.tickets_by_priority || {}).map(([priority, count]) => ({
...
const totalTicketsPriority = computed(() =>
  ticketPriorityData.value.reduce((acc, curr) => acc + curr.count, 0),
)
...
const recentUsers = computed(() => [...allUsers.value].sort((a, b) => b.id - a.id).slice(0, 5))
...
const quickActions = [
  ...
  },
  {
    ...
    desc: 'View and manage tickets',
    to: '/admin/ticket-management',
  },
  {
    label: 'Problem Categories',
    icon: 'category',
    desc: 'Manage issue categories',
